  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 py-6">
      {promoData.map((promo) => (
        <div
          key={promo.id}
          className={`relative h-48 overflow-hidden bg-gradient-to-r ${promo.bgColor} flex items-center group cursor-pointer hover:shadow-lg transition-all duration-300`}
          style={{
        ...
        >
          {/* Content */}
          <div className={`relative z-10 text-left p-6 ${promo.textColor} w-1/2`}>
            <p className="text-xs font-medium mb-1 opacity-80">{promo.title}</p>
            <h3 className="text-xl font-bold mb-2">{promo.subtitle}</h3>
            <button className="bg-white text-gray-900 px-4 py-1.5 text-sm font-medium hover:bg-gray-100 transition-colors duration-300">
              {promo.buttonText}
            </button>
          </div>
        </div>
